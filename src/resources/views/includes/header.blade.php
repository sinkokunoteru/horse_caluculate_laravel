...
<div class="msg">
    <div class="relative flex items-top justify-center min-h-screen bg-gray-100 dark:bg-gray-900 sm:items-center py-4 sm:pt-0">
        @if (Route::has('login'))
            <div class="hidden fixed top-0 right-0 px-6 py-4 sm:block">
                ...
                @else
                    <a href="{{ route('') }}" class="text-sm text-gray-700 dark:text-gray-500 underline">ログイン</a>
                        <a href={{ route('') }} class="ml-4 text-sm text-gray-700 dark:text-gray-500 underline">新規登録</a>
                @endauth
            </div>
        @endif
    </div>
    <nav>  
        <ul class="nav-list ">           
            <li class=”current”><a href={{ route('register') }} class="nav-list-item navI">HOME</a></li>
